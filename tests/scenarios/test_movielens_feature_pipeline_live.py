...
DAG_ID = "movielens_feature_pipeline"
PROVENANCE_KEYS = (
...
def _snapshot_table(table: str) -> dict:
    spec = importlib.util.spec_from_file_location("movielens_live_exec", ROOT / "tests/scenarios/live_exec.py")
...
def test_movielens_feature_pipeline_live_acceptance():
    ...
    with _owned_stack():
        ...
        with _paused_dag():
            ...
            jar = APP / "target/movielens-feature-pipeline-0.1.0.jar"
            ...
            key = "movielens-feature-pipeline/0.1.0/app.jar"
            ...
            first = {
                "user": _snapshot_table("lakehouse.gold.ml_user_features"),
                "movie": _snapshot_table("lakehouse.gold.ml_movie_features"),
                "user_properties": _properties("ml_user_features"),
                "movie_properties": _properties("ml_movie_features"),
            }
            _assert_owned_runs(_airflow, window_start, {first_run_id}, baseline=baseline)
            ...
            assert first["movie"]["schema"] == sorted(
                ["movieId:long", "movie_avg:double", "popularity:long"]
            )
            expected_properties = {
            ...
            assert first["user_properties"] == first["movie_properties"] == expected_properties
            user_measures = _trino(
                "SELECT count(*), sum(num_ratings), min(avg_rating), max(avg_rating) "
                "FROM lakehouse.gold.ml_user_features"
            )[0]
            movie_measures = _trino(
                "SELECT count(*), sum(popularity), min(movie_avg), max(movie_avg) "
                "FROM lakehouse.gold.ml_movie_features"
            )[0]
