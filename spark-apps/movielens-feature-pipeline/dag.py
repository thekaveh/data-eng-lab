"""Airflow DAG: build resolver-verified MovieLens user and movie feature tables."""
...
def _resolve_dataset(dataset: str, scale: str) -> Resolution:
    ...
    try:
        ...
        expected = EXPECTED_OBJECTS[scale]
        if not isinstance(objects, list) or not objects or len(objects) != len(expected):
            raise ValueError
        if any(
        ...
        schemas = tuple(item["schema_id"] for item in objects)
        if names != tuple(item[0] for item in expected) or schemas != tuple(item[1] for item in expected):
            raise ValueError
...
spark_conf = {
    ...
    "spark.app.name": "movielens-feature-pipeline",
...
with DAG(
    ...
    description="Daily resolver-verified MovieLens user and movie feature replacement.",
