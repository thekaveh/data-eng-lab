...
It also deletes the `events/`, `gh_events_file/`, `event_windows/`, and `online_retail_cdc/` prefixes from the MinIO `checkpoints` bucket before their owning streaming formats run. This is an **exclusive-test-only** reset and never a retention eligibility decision. In particular, `gh_events_file/` is a family-root deletion rather than the exact generation leaf required by the [checkpoint retention policy](checkpoint-retention.md). Do not use the gate on a shared environment; it must not run on a shared environment where those tables or prefixes contain state you need to preserve. Issue #86 must replace this root-credential reset with the lease/tombstone/scoped-credential implementation; scheduling remains disabled. It does not edit Atlas source, drop unrelated tables, or delete unrelated MinIO objects.
