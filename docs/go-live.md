...
Streaming scenarios are different: the gate preserves their output tables and every checkpoint object. Both notebook projections acquire the exact writer lease before starting, heartbeat while active, stop if lease ownership is lost, and terminalize in `finally`. The bounded acceptance transform replaces only `query.awaitTermination()` with `query.processAllAvailable()` so the notebook-owned cleanup still runs. It never uses a MinIO root credential, calls a checkpoint delete API, or deletes `events/`, `gh_events_file/`, `event_windows/`, or `online_retail_cdc/`. A reset must instead use issue #86's reviewed exact-leaf retention plan and manual plan/prepare/apply protocol; durable active state and family roots remain ineligible. The separate `checkpoint_retention` Airflow DAG is paused, has `schedule=None`, and performs dry-run planning only. Automatic and scheduled destructive apply remain disabled pending stronger MinIO cross-process CAS and conditional delete proof.
