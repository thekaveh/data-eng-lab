...
atlas `0644a8f3865f9c42dfbe0a279e44a69fdc633c8a`) and **never edits it** —
...
- **We now also need `%trino`** for the Trino scenarios — see A7. Today the Trino interpreter is seeded by Atlas (no manual UI step needed per our agreement).
...
- `data-eng` track = `[spark, airflow, jupyterhub, zeppelin, minio, iceberg-rest, trino, redpanda, supavisor, weaviate, neo4j]`. New launch flags `--iceberg-rest-source`, `--jenkins-source`, `--trino-source`, `--redpanda-source` (all default `disabled`). `supavisor` is an optional Postgres pooler (not a lakehouse dependency).
...
| Multi-engine read | `federated_query`, `bi_query` | **A7 (Trino)** | see section 2 |
...
3. **Zeppelin is standalone Spark, not Spark Connect.** (section A3)
...
5. **Jupyter uses the `jupyter`-scoped MinIO account**, which can't write `lakehouse` via PyIceberg. (section A4)
6. **Jenkins ships no jobs/alias** — downstream provides. (section A5)
...
8. **Zeppelin Trino interpreter is `%trino`, not `%jdbc(trino)`** — Zeppelin 0.12.1 uses the interpreter name as the paragraph prefix. (section 2, Divergence 1)
9. **Trino user is `atlas` (no auth).** Convention; any user accepted. (section 2, Divergence 2)
10. **Redpanda demo-topic seed only includes `atlas_stream_events`.** Downstream projects override `REDPANDA_DEMO_TOPICS` in `.env` or rely on `auto_create_topics_enabled`. (section 2, Divergence 3)
...
- **`go-live.md`** — the runbook that brings the stack up and drives all of the above.
...
Spark status-poll limitation is resolved in the current pin by the #876 correction to the #792
...
hook's driver ID through `spark-master:6066`. The helper raises unless the driver is `FINISHED`
with `success: true`; it is not a false-success workaround.
