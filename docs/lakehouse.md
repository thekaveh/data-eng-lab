...
| `checkpoints/` | Structured streaming checkpoint state. The [checkpoint retention runbook](checkpoint-retention.md) defines exact ownership, leases, manual-only reviewed exact-leaf deletion, and the disabled automatic-scheduling boundary from issue #86. |
...
| Airflow | SparkSubmitOperator (cluster mode) + RestConfirmingSparkHook adapter, DAG orchestration | `s3a://jars/` for JARs, `s3a://lakehouse/` for Iceberg |
...
- [Checkpoint Retention](checkpoint-retention.md) — issue #86 manual-only enforcement is live-accepted; automatic and scheduled destructive apply remain disabled
